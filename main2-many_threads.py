...
class BarcodeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SSCC (GS1-128) PDF Generator — Многопоточная версия")
    ...
    def process_file(self):
        try:
            ...
            raw_codes = df.iloc[:, 0].dropna().astype(str).tolist()
            ...
            for idx, code in enumerate(raw_codes):
                ...
                if not numeric_code.isdigit() or len(numeric_code) != 18:
                    raise ValueError(f"Неверный код в строке {idx + 1}: {code}")
            ...
            self.update_status("Генерация штрих-кодов в потоках...")
